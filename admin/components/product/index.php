...
if (isset($_GET['search']) && $_GET['search'] != "") {
    $key = $_GET['search'] . "%";
    $sql = "SELECT p.id,p.name,c.name as category,s.name as subcategory,p.price,p.availability from product p join category c on p.category=c.id AND c.isactive=:c_active LEFT JOIN category s on p.subcategory=s.id AND s.isactive=:s_active WHERE p.isactive=:p_active AND p.name LIKE :name ORDER BY p.name ";
    $q = $connect->prepare($sql);
    $q->execute([':c_active' => 1, ':s_active' => 1, ':p_active' => 1,':name' => $key]);
                    ...
                    <form action="" method="GET" class="">
                        <div class="row mb-3">
                            <div class="col-md-6 offset-md-2">
                                <input type="text" class="form-control" name="srh" placeholder="Search product ...">
                            </div>
                            <div class="col-md-2">
