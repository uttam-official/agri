...
if (isset($_GET['search']) && $_GET['search'] != "") {
    $key = $_GET['search'] . "%";
    $sql = "SELECT c.id,c.name,c.categoryorder,c.isactive,COUNT(s.id) AS no_of_subcategory  FROM category c LEFT JOIN category s ON c.id=s.parent AND s.isactive>:s_active WHERE c.isactive>:c_active AND c.parent=:parent AND c.name like :name GROUP BY c.id ORDER BY c.name";
    $q = $connect->prepare($sql);
    $q->execute([':s_active' => -1, ':c_active' => -1, ':parent' => 0,':name' => $key]);
                                ...
                                <tbody>
                                    ...
                                    ?>
                                            <tr class="text-center">
                                                ...
                                                <td>
                                                    <a href="<?= BASE_URL ?>/components/category/add.php?id=<?= $value->id ?>" class="btn btn-sm btn-outline-info" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i></a>
                                                    <a href="<?= BASE_URL ?>/components/category/delete.php?id=<?= $value->id ?>" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" title="Delete"> <i class="fa fa-trash"></i></a>
                                                </td>
                                            </tr>
                                        <?php endforeach;
                                    else : ?>
